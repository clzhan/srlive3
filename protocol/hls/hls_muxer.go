...
func (hm *hlsMuxer) segmentOpen(segmentStartDts int64) (err error) {
	...
	if ostype == "windows" {
		hm.current.fullPath = hm.hlsPath + "\\" + "hlsstream" + "\\" + hm.app + "\\" + filename
	} else {
		hm.current.fullPath = hm.hlsPath + "/" + "hlsstream" + "/" + hm.app + "/" + filename
	}
...
func (hm *hlsMuxer) segmentClose(logDesc string) (err error) {
	...
	if hm.current.duration*1000 >= hlsSegmentMinDurationMs {
		...
		log.Println(".......duratuin %v", hm.current.duration)
		...
		//先关闭
		if err = hm.current.muxer.close(); err != nil {
			log.Println("close hls muxer failed, err=", err)
			return
		}
...
func (hm *hlsMuxer) refreshM3u8() (err error) {
	...
	var m3u8File string
	if ostype == "windows" {
		m3u8File = hm.hlsPath + "\\" + "hlsstream" + "\\" + hm.app + "\\" + hm.stream + ".m3u8"
	} else {
		m3u8File = hm.hlsPath + "/" + "hlsstream" + "/" + hm.app + "/" + hm.stream + ".m3u8"
	}
...
func (hm *hlsMuxer) createDir() (err error) {
	...
	var appDir string
	if ostype == "windows" {
		appDir = hm.hlsPath + "\\" + "hlsstream" + "\\" + hm.app
	} else {
		appDir = hm.hlsPath + "/" + "hlsstream" + "/" + hm.app
	}
