...
import (
	...
	"github.com/clzhan/srlive3/protocol/hls"
...
func (s *RtmpNetStream) StreamObject() *stream.StreamObject {
	return s.obj
}
...
// 	return nil
// }
func (s *RtmpNetStream) sendVideo(video *avformat.MediaFrame) error {
	if s.vkfsended {
	...
	if fakf == nil {
		log.Warn("No Video firstAudioKeyFrame", video)
	} else {
...
func (s *RtmpNetStream) sendAudio(audio *avformat.MediaFrame) error {
	if !s.vkfsended {
		return nil
	}
	if s.akfsended {
...
func (s *RtmpNetStream) sendMetaData(data *avformat.MediaFrame) error {
	...
	for {
		...
		if am, ok := msg.(*AudioMessage); ok {
			...
			if strings.HasSuffix(conn.app, "/") {
				...
				log.Info("1....app:", conn.app, "stream :", strings.Split(m.PublishName, "?")[0])
			} else {
				s.path = conn.app + "/" + strings.Split(m.PublishName, "?")[0]
				log.Info("2....app:", conn.app, "stream :", strings.Split(m.PublishName, "?")[0])
			}
			s.streamName = s.path
			...
			if s.mode == 0 {
				...
				s.mode = s.mode | rtmpconst.MODE_PRODUCER
			}

			//添加HLS
			//TODO
			obj, found := stream.FindObject(s.path)
			if found {
				stream := hls.NewHttpHlsStream(s.path)
				stream.OnPublish(conn.app, strings.Split(m.PublishName, "?")[0])
				stream.SetObj(obj)
				obj.HttpHlsAttach(stream)
				log.Info("HttpHlsStream HttpHlsAttach......")
				go stream.WriteLoop()
			}
			...
			if s.mode == 0 {
				...
				s.mode = s.mode | rtmpconst.MODE_CONSUMER
			}
		} else if m, ok := msg.(*CloseStreamMessage); ok {
			log.Debug("IN", " <<<<<<< ", m)
			s.Close()
		} else if m, ok := msg.(*DeleteStreamMessage); ok {
			log.Debug("IN hsh add Why?", " <<<<<<< ", m)
			s.Close()
		} else {
